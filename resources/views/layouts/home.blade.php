...
@section('content')
    ...
    <ul class="rank">
        @foreach($new as $n)
            <li><a href="{{url('a/'.$n->id)}}" title="{{$n->article_title}}" target="_blank">{{$n->article_title}}</a></li>
        @endforeach
    </ul>
    <h3 class="ph">
        <p>点击<span>排行</span></p>
    ...
    <ul class="paih">
        @foreach($hot as $h)
            <li><a href="{{url('a/'.$h->id)}}" title="{{$h->article_title}}" target="_blank">{{$h->article_title}}</a></li>
        @endforeach
    </ul>
@show
